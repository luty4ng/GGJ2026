...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class EntityComponent : GameFrameworkComponent
    {
        ...
        private const int k_IdGenCount = 50;
        private int m_serialId = 0;
        public int GenerateEntityId()
        {
            int count = 0;
            m_serialId++;
            while (HasEntity(m_serialId))
            {
                // 如果不是第一次，并且序列ID尝试次数超出上限
                if (m_serialId != 1 && count >= k_IdGenCount)
                {
                    Debug.LogError($"生成EntityId超时, 当前序列ID: {m_serialId}");
                    break;
                }
                m_serialId++;
            }

            Debug.Log("生成EntityId: " + m_serialId);
            return m_serialId;
        }
        public void ShowEntity(Type logicType, string assetName, string groupName, object userData = null)
        {
            int serialId = GenerateEntityId();
            ShowEntity(serialId, logicType, assetName, groupName);
        }

        public void ShowEntity<T>(string assetName, string groupName, object userData = null) where T : EntityLogic
        {
            ShowEntity(typeof(T), assetName, groupName, userData);
        }

        public void ShowEntityInDefaultGroup(Type logicType, string assetName, object userData = null)
        {
            int serialId = GenerateEntityId();
            ShowEntity(serialId, logicType, assetName, "DefaultEntityGroup");
        }

        public void ShowEntityInDefaultGroup<T>(string assetName, object userData = null) where T : EntityLogic
        {
            ShowEntityInDefaultGroup(typeof(T), assetName, userData);
        }

        public async UniTask<T> ShowEntityAsync<T>(string entityAssetName, string entityGroupName, object userData = null) where T : class
        {
            int serialId = GenerateEntityId();
            return await ShowEntityAsync<T>(serialId, entityAssetName, entityGroupName, userData);
        }
