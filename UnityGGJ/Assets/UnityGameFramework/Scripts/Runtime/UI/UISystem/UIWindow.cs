...
namespace UnityGameFramework.Runtime
{
    public abstract class UIWindow : UIBase
    {
        ...
        private bool _resetTransformOnLoad = true;
        ...
        public void Init(string name, int layer, bool fullScreen, string assetName, bool fromResources, int hideTimeToClose, bool resetTransformOnLoad = true)
        {
            WindowName = name;
            WindowLayer = layer;
            ...
            _resetTransformOnLoad = resetTransformOnLoad;
        }
        ...
        internal async UniTaskVoid InternalLoad(string location, Action<UIWindow> prepareCallback, bool isAsync, System.Object[] userDatas)
        {
            ...
            { 
                if (isAsync)
                {
                    var uiInstance = await GameModule.Resource.LoadGameObjectAsync(location, parent: GameModule.UI.UICanvasTransform);
                    Handle_Completed(uiInstance);
                }
                else
                {
                    var uiInstance = GameModule.Resource.LoadGameObject(location, parent: GameModule.UI.UICanvasTransform);
                    Handle_Completed(uiInstance);
                }
            }
            else
            {
                GameObject panel = Object.Instantiate(Resources.Load<GameObject>(location), GameModule.UI.UICanvasTransform);
