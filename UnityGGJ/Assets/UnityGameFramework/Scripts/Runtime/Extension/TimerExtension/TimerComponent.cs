...
public class TimerComponent : GameFrameworkComponent
{
    ...
    private void RunTimer(Timer timer)
    {
        switch (timer.TimerType)
        {
            ...
            case TimerType.RecyclableOnceWait:
                {
    ...
    /// <returns></returns>
    public async UniTask<bool> OnceTimerAsync(long time, Action<long> updateCallBack = null, CancellationToken cancellationToken = null)
    {
        long nowTime = TimerTimeUtility.Now();
        if (time <= 0)
        {
        ...
        Timer timer = Timer.Create(time, nowTime, TimerType.OnceWait, tcs, 0, updateCallBack);
        m_Timers.Add(timer.ID, timer);
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public async UniTask<bool> RecyclableOnceTimerAsync(long time, Action<long> updateCallBack = null, CancellationToken cancellationToken = null)
    {
        long nowTime = TimerTimeUtility.Now();
        if (time <= 0)
        {
        ...
        Timer timer = Timer.Create(time, nowTime, TimerType.RecyclableOnceWait, tcs, 0, updateCallBack);
        m_Timers.Add(timer.ID, timer);
        int timerId = timer.ID;
    ...
    {
        return await OnceTimerAsync(1, null, cancellationToken);
    }
